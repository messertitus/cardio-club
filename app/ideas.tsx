...
export default function IdeasScreen() {
  ...
  const [allSports, setAllSports] = useState<Row<"sports">[]>([]);
  ...
  const [modalSportSearch, setModalSportSearch] = useState("");
  const [sportChoiceLimit, setSportChoiceLimit] = useState(10);
  ...
  async function load() {
    if (!user) return;
    setBusy(true);
    const [sportsResult, allSportsResult, profilesResult, linksResult, ideasResult, adminResult] = await Promise.all([
      listSports(supabase),
    ...
    if (allSportsResult.data) setAllSports(allSportsResult.data.filter((sport) => sport.is_active !== false));
    ...
    if (sportsResult.error || allSportsResult.error || profilesResult.error || linksResult.error || ideasResult.error || adminResult.error) {
      setMessage(sportsResult.error?.message ?? allSportsResult.error?.message ?? profilesResult.error?.message ?? linksResult.error?.message ?? ideasResult.error?.message ?? adminResult.error?.message ?? null);
    }
  }
  ...
  const activeSport = useMemo(() => allSports.find((sport) => sport.id === activeSportId) ?? null, [activeSportId, allSports]);
  ...
  const filteredFormSports = useMemo(() => {
    const query = modalSportSearch.trim().toLowerCase();
    const source = allSports.filter((sport) => sport.is_active !== false);
    if (!query) return source;
    return source.filter((sport) => `${sport.name} ${sport.category ?? ""}`.toLowerCase().includes(query));
  }, [allSports, modalSportSearch]);
  const visibleFormSports = filteredFormSports.slice(0, sportChoiceLimit);
  const hasMoreFormSports = visibleFormSports.length < filteredFormSports.length;
  ...
  const sportCatalog = allSports.length ? allSports : sports;
  ...
  async function saveDraft() {
    ...
    setBusy(true);
    const sportCatalog = allSports.length ? allSports : sports;
    const input = draftToInput(draft, user.id, persistedDraftStep(activeStep), sportCatalog, activeSportId ?? draft.sportId);
    ...
    setDraft(ideaToDraft({ ...result.data, creatorName: "Du", creatorCity: null, sportName: selectedSportName(result.data.sport_id, sportCatalog) }));
  ...
  async function submit() {
    ...
    if (sportIdsToSubmit.length > 0) {
      for (const sportId of sportIdsToSubmit) {
        ...
        const result = await submitSportIdea(supabase, draftToInput(inputDraft, user.id, "review", sportCatalog, sportId));
        if (result.error) {
      ...
      const result = await submitSportIdea(supabase, draftToInput(draft, user.id, "review", sportCatalog));
  ...
  function toggleSportChoice(sport: Row<"sports">) {
    ...
    setDraft((current) => {
      ...
      const firstSportName = selectedSportName(firstSportId, allSports) ?? "";
        ...
        <KeyboardAvoidingView behavior={undefined} style={styles.shell}>
          <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
            ...
            <View style={[styles.card, styles.formSheet, { borderColor: theme.border, backgroundColor: theme.surface }]}>
                ...
                <View style={[styles.subflowPanel, { backgroundColor: theme.softSurface }]}>
                  ...
                  <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.subflowChipRow}>
                    {selectedSportIds.map((sportId) => {
                      const sport = allSports.find((entry) => entry.id === sportId);
              ...
              {activeStep === "sport" ? (
                <View style={styles.formGrid}>
                  <SearchField value={modalSportSearch} onChangeText={(value) => {
                    setModalSportSearch(value);
                    setSportChoiceLimit(10);
                  }} placeholder="Sportart suchen" />
                  <View style={styles.choiceGrid}>
                    {visibleFormSports.map((sport) => {
                    ...
                    {hasMoreFormSports ? (
                      <Pressable style={[styles.choiceChip, styles.plusChoice, { backgroundColor: theme.surface }]} onPress={() => setSportChoiceLimit((limit) => limit + 10)}>
                        <Text style={[styles.choiceText, { color: theme.text }]}>...</Text>
                      </Pressable>
                    ) : null}
                  ...
                  {filteredFormSports.length === 0 ? <Text style={[styles.ideaNote, { color: theme.muted }]}>Keine Sportart gefunden.</Text> : null}
              ...
              {activeStep === "review" ? (
                <View style={styles.formGrid}>
                  <DetailLine label="Sportart" value={ideaNameFromDraft(draft, sportCatalog) || "Noch offen"} />
                  <DetailLine label="Profil" value={buildProfileName(draft, sportCatalog)} />
                  ...
                  {selectedSportIds.length > 1 ? (
                    <View style={styles.reviewSportList}>
                      {selectedSportIds.map((sportId) => {
                        ...
                        const sport = sportCatalog.find((entry) => entry.id === sportId);
                        return (
...
function isSportSpecificStep(step: IdeaFlowStep): boolean {
  return step === "type" || step === "group" || step === "weather" || step === "equipment" || step === "available" || step === "schedule" || step === "logistics";
}
...
function ideaToDraft(idea: Row<"sport_ideas"> & Partial<SportIdeaWithCreator>): IdeaDraft {
  ...
  return {
    ...
    sportDetails: sportId ? { [sportId]: sportDetail } : {},
...
function draftToInput(draft: IdeaDraft, userId: string, draftStep: SportIdeaDraftStep, sports: Row<"sports">[], sportIdOverride?: string | null) {
  ...
  const detail = sportDetailForDraft(draft, sportId);
  const name = ideaNameFromDraft(draft, sports, sportId);
  const profileName = buildProfileName(draft, sports, sportId);
  return {
    ideaId: draft.ideaId,
...
function requiredErrors(draft: IdeaDraft): Record<string, string> {
  ...
  if (selectedIds.length === 0 && !draft.requestedSportName.trim()) errors.sport = "Wähle eine Sportart aus oder frage eine neue an.";
...
const styles = StyleSheet.create({
  ...
  reviewSportCard: { borderRadius: 16, gap: 7, padding: 10 },
  reviewSportTitle: { fontSize: 13, fontWeight: "900" },
