-- 037: Two events per week — a Saturday and a Sunday Cardiotag.
--
-- The fairness algorithm is unchanged; it simply runs per event as before. This
-- migration lets a club hold one event on Saturday and one on Sunday in the same
-- week, each with its own voting, attendance and decision.

-- 1. Day marker on events (existing rows become 'sunday').
...
-- 2. Allow two events per club per week (one per day).
...
-- 3. Provision both days for the current MCC week and return every event of the
--    active week. While a previous week still has an open event, keep showing
--    that week (no new voting until it is closed).
...
begin
  ...
  -- Gate: keep the most recent still-open prior week visible until it is closed.
  select max(we.week_start_date)
  into open_previous_week
  ...
  if open_previous_week is not null then
    return query
      select resolved_club_id, we.id, we.event_day
      from public.weekly_events we
      where we.club_id = resolved_club_id
        and we.week_start_date = open_previous_week
      order by we.event_day;
    return;
  end if;
  ...
  values
    (
      ...
      'voting',
      'Seepark Freiburg',
      (current_week::timestamp + interval '5 days 11 hours')::timestamptz,
      'Cardio-Samstag. Teilnahme und Voting laufen unter der Woche.',
      null,
      'saturday'
    ),
    (
      ...
      'Cardio-Sonntag. Teilnahme und Voting laufen unter der Woche.',
      null,
      'sunday'
    )
